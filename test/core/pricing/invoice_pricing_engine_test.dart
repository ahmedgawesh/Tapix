...
InvoicePricingResult _compute({
  ...
  bool taxInclusive = false,
    ...
    InvoicePricingEngine.compute(InvoicePricingInput(
      lines: lines,
...
void main() {
  ...
  group('InvoicePricingEngine — bug regression: 1 % per-item == 1 % overall', () {
    ...
    test('overall 1 % discount on the same line → MUST equal 799.92', () {
      ...
      expect(r.subtotal.cents, 80000);
      expect(r.itemDiscountTotal, Money.zero);
      expect(r.overallDiscount.cents, 800,
    ...
    test('per-item == overall for many random shapes (parity invariant)', () {
      ...
      for (final shape in shapes) {
        ...
        expect(overall.total.cents, perItem.total.cents,
            reason: 'parity broken for shape $shape');
        expect(overall.tax.cents, perItem.tax.cents,
  ...
  group('InvoicePricingEngine — proration of overall discount', () {
    ...
    test('shares always sum exactly to overall discount (no rounding drift)', () {
  ...
  group('InvoicePricingEngine — invariants', () {
    test('subtotal − totalDiscount + tax == total (always)', () {
      final cases = <Map<String, dynamic>>[
        {
          'lines': [
            _line(unitPriceCents: 10000, qty: 2, isTaxable: true, productTaxRateBps: 1500),
            _line(unitPriceCents: 5000, qty: 3, isTaxable: true, productTaxRateBps: 1500),
          ],
          'overall': Discount.percent(500),
        },
        {
          'lines': [
            _line(unitPriceCents: 12345, qty: 7, isTaxable: true, productTaxRateBps: 100),
          ],
          'overall': Discount.fixed(Money.fromCents(1234)),
        },
        {
          'lines': [
            _line(unitPriceCents: 99, qty: 100),
          ],
    ...
    test('Σ line.total == invoice.total (no escaped cents)', () {
      final r = _compute(
        lines: [
          _line(unitPriceCents: 333, qty: 5, isTaxable: true, productTaxRateBps: 1500),
          _line(unitPriceCents: 777, qty: 3, isTaxable: true, productTaxRateBps: 1500),
          _line(unitPriceCents: 1111, qty: 1, isTaxable: true, productTaxRateBps: 1500),
        ],
        overall: Discount.percent(123),
      );
      final lineSum =
          r.lines.fold<int>(0, (s, l) => s + l.total.cents);
      expect(lineSum, r.total.cents);
    });

    test('quantity change reflects on overall percent (closes bug #2 invoice-side)', () {
  ...
  group('InvoicePricingEngine — tax disabled', () {
    test('enableTax=false zeros all tax', () {
      final r = _compute(
        lines: [_line(unitPriceCents: 10000, qty: 1, isTaxable: true, productTaxRateBps: 1500)],
        overall: Discount.percent(100),
        enableTax: false,
      );
      expect(r.tax, Money.zero);
