...
void main() {
  ...
  group('Phase 4 — engine SoT migration (sale form)', () {
    ...
    test('Q3c — invoice-mode fixed discount > subtotal: engine clamps to '
    ...
    test('Q5a — totalBeforeLoyalty == subtotal − totalDiscount + tax '
        '(clamped ≥ 0)', () {
    ...
    test(
      'Q5b — Σ engine.line.total == invoice.total exactly (no lost cents)',
      () {
        final s = _state(
          items: [
            _line(
    ...
    test('Q2a — loyaltyDiscount does NOT reduce pricing.total '
    ...
    test('Q2b — totalCents == max(0, pricing.total − loyaltyDiscountCents) '
    ...
    // ── Tender invariants ────────────────────────────────────────────────
    test(
      'remaining = max(0, totalCents − paid); change = max(0, paid − total)',
      () {
        final s = _state(
          items: [
            _line(
    ...
    test('SaleLineItem.taxCentsWithSettings honors global toggle + '
    ...
    test('loyalty redemption never alters subtotal / tax / pricing.total — '
