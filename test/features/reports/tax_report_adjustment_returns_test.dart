...
void main() {
  ...
  Future<T> firstSuccess<T>(Stream<RealtimeState<T>> stream) async {
    final state = await stream
            .firstWhere((s) => s is RealtimeSuccess<T>)
  ...
  test('sales tax report sums VAT of linked AND adjustment returns', () async {
    ...
    await db.into(db.saleReturnAdjustments).insert(
  ...
  test('purchase tax report sums VAT of linked AND adjustment returns',
      () async {
    ...
    await db.into(db.purchaseReturnAdjustments).insert(
