...
void main() {
  ...
  group('ProductFormBloc', () {
    ...
    setUp(() {
      ...
      when(() => repository.getCostingMethodLockReason(any()))
          .thenAnswer((_) async => null);
      ...
      when(() => repository.findByBarcode(any()))
          .thenAnswer((_) async => null);
      when(() => repository.findBySku(any()))
          .thenAnswer((_) async => null);
      when(() => variantRepository.getVariantByBarcode(any()))
          .thenAnswer((_) async => null);
      when(() => variantRepository.getVariantBySku(any()))
          .thenAnswer((_) async => null);
    });
    ...
    group('ProductFormInitialized', () {
      ...
      test('new product inherits defaultMinQuantity and defaultTrackInventory from settings', () async {
        bloc.add(const ProductFormInitialized(
          productId: null,
      ...
      test('new product can be initialized with tracking disabled by default', () async {
        bloc.add(const ProductFormInitialized(
          productId: null,
    ...
    group('ProductFormSubmitted', () {
      ...
      test(
        ...
        bloc.add(ProductFormFieldChanged(
            field: 'costCents', value: Decimal.fromInt(100)));
        bloc.add(ProductFormFieldChanged(
            field: 'priceCents', value: Decimal.fromInt(200)));
        bloc.add(const ProductFormFieldChanged(
            field: 'inventoryTrackingType', value: 'batch_expiry'));
      ...
      test(
        ...
        bloc.add(ProductFormFieldChanged(
            field: 'priceCents', value: Decimal.fromInt(200)));
        bloc.add(const ProductFormFieldChanged(
            field: 'inventoryTrackingType', value: 'batch'));
