...
void main() {
    ...
    return db
        ...
        .insert(
          ProductVariantsCompanion.insert(
            ...
            // Deletion is valid only after all stock has been settled.
  ...
  test(
    'smartDeleteProduct hard-deletes when there are no references',
    () async {
      ...
      expect(
        row,
        isNull,
        reason: 'row must be removed when no references exist',
      );
    },
  );

  test(
    'smartDeleteProduct soft-deletes when there are sale/purchase refs',
    () async {
      final productId = await insertProduct();
      ...
      final refCountBefore = await db.productDao.countProductReferences(
        productId,
      );
      ...
      expect(
        ...
        isFalse,
        reason:
            'product with references must be soft-deleted to preserve audit trail',
      );
