...
void main() {
  ...
  test('account 4900 Purchase Discounts Earned is seeded as revenue', () async {
    final acct = await (db.select(
      db.accounts,
    )..where((a) => a.accountCode.equals('4900'))).getSingleOrNull();
    expect(
      acct,
      isNotNull,
  ...
  test(
    ...
    () async {
      ...
      final entries = await (db.select(
        db.journalEntries,
      )..where((e) => e.entryType.equals('supplier_discount'))).get();
      expect(
        entries,
        hasLength(1),
      ...
      ])..where(db.journalEntryLines.journalEntryId.equals(entryId))).get();
      ...
      expect(
        ...
        equals({'2000', '4900'}),
        reason:
            'JE must touch exactly AP (2000) and Purchase Discounts '
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        equals(glPre),
        reason:
            'GL Inventory must NOT move when a supplier discount is '
  ...
  test('LedgerRebuildService produces correct accounting on rebuild', () async {
