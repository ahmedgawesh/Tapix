...
void main() {
  ...
  group('Bug 2: price_history records purchase price, not post-WAC cost', () {
    test('WAC blends variant.cost to 52.50 but audit log shows 60.00', () async {
      ...
      final variant = await (db.select(db.productVariants)
            ..where((v) => v.id.equals(ctx.variantId)))
          .getSingle();
      expect(
      ...
      //
      // Storage convention (matches `ProductLocalDatasource.getPriceHistory`):
      // the `*_cents` columns actually store Decimal *dollars* (cents ÷ 100)
      // and consumers shift(2) on read. We therefore use the typed Drift
      // select and shift back to cents for the assertions.
      final histories = await (db.select(db.productPriceHistories)
            ..where((h) =>
      ...
      expect(row.oldCostCents.shift(2).toBigInt().toInt(), equals(5000),
          reason: 'Old cost must equal the variant.cost before WAC mutation');
      expect(row.newCostCents.shift(2).toBigInt().toInt(), equals(6000),
          reason:
    ...
    test('per-line discount: audit log shows GROSS typed cost, not NET', () async {
      ...
      final variant = await (db.select(db.productVariants)
            ..where((v) => v.id.equals(ctx.variantId)))
          .getSingle();
      expect(variant.costCents.toBigInt().toInt(), equals(9900),
          reason:
      ...
      final row = histories.first;
      expect(row.newCostCents.shift(2).toBigInt().toInt(), equals(10000),
          reason:
    ...
    test('consecutive purchases compare GROSS-to-GROSS in the audit log', () async {
      ...
      final po1 = await db.into(db.purchases).insert(
        PurchasesCompanion.insert(
      ...
      final po2 = await db.into(db.purchases).insert(
        PurchasesCompanion.insert(
      ...
      final rows = await (db.select(db.productPriceHistories)
            ..where((h) =>
                h.productId.equals(ctx.productId) &
                h.variantId.equals(ctx.variantId))
      ...
      expect(rows[0].newCostCents.shift(2).toBigInt().toInt(), equals(10000),
          reason: 'First row records GROSS 10000 (the typed cost)');
      expect(rows[1].oldCostCents.shift(2).toBigInt().toInt(), equals(10000),
          reason:
      ...
      expect(rows[1].newCostCents.shift(2).toBigInt().toInt(), equals(12000),
          reason: 'Second row records GROSS 12000');
    });

    test('WAC does NOT alter variant cost when product is non-tracked', () async {
      ...
      final variant = await (db.select(db.productVariants)
          ...
          .getSingle();
      expect(variant.costCents.toBigInt().toInt(), equals(5000),
          reason: 'Non-tracked products must not have their cost mutated by WAC');
      ...
      final histories = await db.customSelect(
      ...
      expect(histories.length, equals(0),
          reason: 'No price-history row should be written when nothing changed '
  ...
  group('Bug 1 (DAO surface): variant wholesale stays NULL when not set', () {
    test('posting a sibling variant does not write a wholesale value to '
      ...
      final rowA = await (db.select(db.productVariants)
            ..where((v) => v.id.equals(variantA)))
          .getSingle();
      expect(rowA.wholesalePriceCents?.toBigInt().toInt(), equals(7000),
      ...
      final rowB = await (db.select(db.productVariants)
            ..where((v) => v.id.equals(variantB)))
          .getSingle();
      expect(rowB.wholesalePriceCents, isNull,
          reason: 'Sibling variant B must NOT inherit Variant A\'s wholesale '
      ...
      final histB = await db.customSelect(
