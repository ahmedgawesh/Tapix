...
void main() {
  ...
  Future<({int debitCents, int creditCents})> accountBalance(String code) async {
  ...
  group('Account classification (contra-accounts IFRS/GAAP)', () {
    test('4100 Purchase Return Adjustment is seeded as expense (contra-COGS)',
        () async {
      final row = await db.customSelect(
        'SELECT account_type FROM accounts WHERE account_code = ?',
        variables: [Variable.withString('4100')],
      ).getSingle();
      expect(row.read<String>('account_type'), equals('expense'),
          reason:
              '4100 must net against COGS in P&L — expense type produces a '
              'negative (contra) figure when the return Dr is on AP.');
    });

    test('5700 Sales Return Adjustment is seeded as revenue (contra-sales)',
        () async {
      final row = await db.customSelect(
        'SELECT account_type FROM accounts WHERE account_code = ?',
        variables: [Variable.withString('5700')],
      ).getSingle();
      expect(row.read<String>('account_type'), equals('revenue'),
          reason:
  ...
  group('writeOffAndDeleteVariant — GL stays in sync', () {
    test(
        'stock>0 → posts Shrinkage Dr 5800 / Cr 1200 for qty × cost, then '
      ...
      expect(row.stockQuantity, equals(0),
      ...
      expect(after1200.creditCents - before1200.creditCents,
          equals(40 * 2500));
    });

    test('stock=0 → short-circuits: no JE posted, smart-delete hard-path',
        () async {
      final productId = await db.into(db.products).insert(
            ProductsCompanion.insert(
  ...
  group('getTotalInventoryValueCents — audit-grade reconciliation', () {
    test('sums Σ(stock × cost) across ALL variants (active AND inactive)',
        () async {
      ...
      final redId = await db.into(db.productColors).insert(
            ProductColorsCompanion.insert(
              name: 'RecRed',
              hexCode: const Value('#AA0000'),
            ),
          );
      final greenId = await db.into(db.productColors).insert(
            ProductColorsCompanion.insert(
              name: 'RecGreen',
      ...
      expect(total, equals(10 * 1000 + 5 * 1000 + 7 * 500),
          reason:
