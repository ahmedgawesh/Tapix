...
void main() {
  ...
  group('TEST 1: Sale creation end-to-end', () {
    ...
    test(
      'cash sale: creates journal entry, deducts stock, no customer balance change',
      () async {
        // Create the sale
  ...
  group('TEST 2: Sale void end-to-end', () {
    ...
    test(
      'void completed credit sale: restores stock and customer balance',
      () async {
        ...
        final preVoidVariant = await (db.select(
          db.productVariants,
        )..where((v) => v.id.equals(variantId))).getSingle();
        expect(preVoidVariant.stockQuantity, equals(98));

        final preVoidCustomer = await (db.select(
          db.customers,
        )..where((c) => c.id.equals(customerId))).getSingle();
        ...
        final postVoidVariant = await (db.select(
          db.productVariants,
        )..where((v) => v.id.equals(variantId))).getSingle();
        expect(
          postVoidVariant.stockQuantity,
          equals(100),
          reason: 'Stock must be restored to original 100',
        );

        // VERIFY: Customer balance restored to 0
        final postVoidCustomer = await (db.select(
          db.customers,
        )..where((c) => c.id.equals(customerId))).getSingle();
        expect(
          postVoidCustomer.balanceCents.toBigInt().toInt(),
          equals(0),
    ...
    test(
      'void draft sale: no journal reversal needed, no stock change',
      () async {
        ...
        final preVariant = await (db.select(
          db.productVariants,
        )..where((v) => v.id.equals(variantId))).getSingle();
        ...
        final postVariant = await (db.select(
          db.productVariants,
        )..where((v) => v.id.equals(variantId))).getSingle();
        expect(
          postVariant.stockQuantity,
          equals(100),
  ...
  group('TEST 3: Purchase posting end-to-end', () {
    ...
    test(
      'credit purchase posting: increases stock, creates journal, updates supplier balance',
      () async {
        // Create a purchase
  ...
  group('Trial balance invariant', () {
    test('after sale + COGS, trial balance sums to zero', () async {
      ...
      expect(
        ...
        equals(totalC),
        reason:
            'Trial balance must always sum to zero (total debits == total credits)',
      );
    });
  });
  ...
  group('REGRESSION: purchase line discount → variant cost net of discount', () {
    ...
    test('purchase with line discount: variant cost = (gross − discount/qty); '
      ...
      expect(
        ...
        reason:
            'Variant cost MUST be the discounted unit cost. '
            'Field bug: was the gross 10000 → produced 3000-cent inventory '
            'GL drift.',
      );

      // ── Inventory GL == Σ(stock × cost) ──
      final invAccountId = await (db.select(
        db.accounts,
      )..where((a) => a.accountCode.equals('1200'))).getSingle();
      final invDr = await db
          .customSelect(
      ...
      final stockValue =
          variant.stockQuantity * variant.costCents.toBigInt().toInt();
      expect(
        inventoryGl,
        equals(stockValue),
        reason:
            'Σ(stock × cost) must equal Inventory GL. '
    ...
    test('purchase with NO line discount: variant cost == gross unit cost '
      ...
      expect(
        ...
        equals(grossUnit),
        reason:
            'No discount: variant cost stays at gross unit cost '
