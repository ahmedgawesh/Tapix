...
class PurchaseAdjReturnDetailScreen extends StatefulWidget {
  final int returnId;

  const PurchaseAdjReturnDetailScreen({super.key, required this.returnId});

  @override
  State<PurchaseAdjReturnDetailScreen> createState() =>
      _PurchaseAdjReturnDetailScreenState();
}

class _PurchaseAdjReturnDetailScreenState
    extends State<PurchaseAdjReturnDetailScreen> {
  PurchaseReturnAdjustment? _returnEntity;
  String? _supplierName;
  List<PurchaseAdjReturnItemWithDetails> _returnItems = [];
  bool _loading = true;
  StreamSubscription<List<PurchaseAdjReturnItemWithDetails>>? _itemsSub;
  ...
  Future<void> _loadData() async {
    ...
    final ret = await dao.getPurchaseAdjReturnById(widget.returnId);
    if (ret == null) {
    ...
    try {
      ...
      final supplier = await (db.select(db.suppliers)
            ..where((s) => s.id.equals(ret.supplierId)))
          .getSingleOrNull();
    ...
    _itemsSub?.cancel();
    _itemsSub = dao
        .watchPurchaseAdjReturnItemsWithDetails(widget.returnId)
        .listen((items) {
      if (mounted) {
        setState(() {
          _returnItems = items;
    ...
    return Scaffold(
              ...
              itemBuilder: (_) => [
                ...
                PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('purchases.share_invoice'.tr()),
                    dense: true, contentPadding: EdgeInsets.zero,
                  ),
                ),
                ...
                PopupMenuItem(
                  ...
                  child: Builder(builder: (context) {
                    ...
                    return ListTile(
                      ...
                      title: Text('purchases.void_return'.tr(),
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSupplierCard(context, ret),
          const SizedBox(height: 12),
  ...
  Future<void> _printPdf(BuildContext context) async {
    ...
    try {
      await PurchasePdfService.printPurchaseAdjReturn(
        context: context,
        returnEntity: ret,
  ...
  Future<void> _sharePdf(BuildContext context) async {
    ...
    final messenger = ScaffoldMessenger.of(context);
    try {
      await PurchasePdfService.sharePurchaseAdjReturn(
        context: context,
        returnEntity: ret,
        returnItems: _returnItems,
        supplierName: _supplierName,
  ...
  Future<void> _voidReturn(BuildContext context) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: Text('purchases.void_return'.tr()),
          ),
        ],
      ),
    );
    ...
    try {
      final dao = sl<AdjustmentReturnDao>();
      await dao.voidPurchaseAdjReturn(
        widget.returnId,
  ...
  Widget _buildReturnInfoCard(BuildContext context,
      PurchaseReturnAdjustment ret, CurrencyService cs) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
  ...
  Widget _buildSupplierCard(
      BuildContext context, PurchaseReturnAdjustment ret) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
    final initial = _supplierName != null && _supplierName!.isNotEmpty
        ? _supplierName![0].toUpperCase()
        : '?';
              ...
              child: Column(
                ...
                children: [
                  Text('purchases.supplier'.tr(),
                      style: theme.textTheme.labelSmall?.copyWith(
                          color: colorScheme.onSurfaceVariant)),
                  Text(_supplierName ?? 'ID: ${ret.supplierId}',
  ...
  Widget _buildReturnItemsCard(BuildContext context,
      List<PurchaseAdjReturnItemWithDetails> items, CurrencyService cs) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
            ...
            if (items.isEmpty)
              ...
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  // Build variant label parts (color · sku)
                              ...
                              if (variantLine != null)
                                Text(variantLine,
                                    style: theme.textTheme.bodySmall?.copyWith(
                                        ...
                                        fontSize: 11)),
                              Text(
                                  '${'purchases.qty'.tr()}: ${d.item.quantity}',
                                  style: theme.textTheme.bodySmall?.copyWith(
                                      color: colorScheme.onSurfaceVariant,
                                      fontSize: 11)),
  ...
  // TOTAL CARD
  // ═══════════════════════════════════════════════════════
  Widget _buildTotalCard(BuildContext context, PurchaseReturnAdjustment ret,
      CurrencyService cs) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
    ...
    return Card(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            _totalRow(
                theme, 'purchases.total_items_count'.tr(), '$totalItems'),
            _totalRow(
                theme, 'purchases.total_pieces_count'.tr(), '$totalPieces'),
            Divider(
                height: 16,
                color: colorScheme.outlineVariant.withValues(alpha: 0.4)),
            if (subtotal > 0)
