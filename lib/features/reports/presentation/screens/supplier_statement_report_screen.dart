...
import '../bloc/supplier_statement_report_bloc.dart';
import '../widgets/date_range_selector.dart';
import '../widgets/searchable_party_selector.dart';

class SupplierStatementReportScreen extends StatelessWidget {
  const SupplierStatementReportScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<SupplierStatementReportBloc>(),
      child: const _SupplierStatementReportView(),
    );
  }
}

class _SupplierStatementReportView extends StatelessWidget {
  const _SupplierStatementReportView();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('reports.supplier_statement_report'.tr()),
        actions: [
          BlocBuilder<
            SupplierStatementReportBloc,
            RealtimeState<SupplierStatementData>
          >(
            builder: (context, state) {
              if (state is! RealtimeSuccess<SupplierStatementData>) {
      ...
      body:
          ...
          >(
            builder: (context, state) {
              if (state is RealtimeLoading<SupplierStatementData>) {
                return const Center(child: CircularProgressIndicator());
              }

              if (state is RealtimeError<SupplierStatementData>) {
                return Center(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: _SupplierSelector(
                        ...
                        onChanged: (id) => context
                            .read<SupplierStatementReportBloc>()
                            .add(SupplierStatementReportSupplierChanged(id)),
                      ),
                    ),
                    ...
                    Padding(
                      ...
                      child: DateRangeSelector(
                        ...
                        onChanged: (range) =>
                            context.read<SupplierStatementReportBloc>().add(
                              SupplierStatementReportDateRangeChanged(range),
                            ),
                      ),
                    ),
                    const SizedBox(height: 8),
  ...
  Future<void> _printReport(
    ...
    SupplierStatementData data,
  ) async {
    await SupplierStatementPdfService.printSupplierStatement(
      context: context,
      data: data,
    );
    sl<AuditLogService>().log(
      entityType: 'report',
      entityId: 0,
      action: 'print_supplier_statement_report',
    );
  }
  ...
  Future<void> _shareReport(
    ...
    await SupplierStatementPdfService.shareSupplierStatement(
      context: context,
      data: data,
    );
    sl<AuditLogService>().log(
      entityType: 'report',
      entityId: 0,
      action: 'share_supplier_statement_report',
    );
  }
}
...
class _StatementContent extends StatelessWidget {
  final SupplierStatementData data;
      ...
      child: DataTable(
        ...
        rows: [
          ...
          ...data.transactions.map((txn) {
            ...
            if (txn.transactionNumber != null) {
              descParts.add(txn.transactionNumber!);
            }
