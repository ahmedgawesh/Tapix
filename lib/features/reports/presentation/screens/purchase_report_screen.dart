...
class _PurchaseReportViewState extends State<_PurchaseReportView> {
  ...
  void _onPrint(BuildContext context, PurchaseReportsData data) {
    switch (widget.reportType) {
      case PurchaseReportType.all:
        PurchaseReportsPdfService.printInvoiceList(context: context, title: _title(), invoices: data.allPurchases, data: data);
      case PurchaseReportType.cash:
        PurchaseReportsPdfService.printInvoiceList(context: context, title: _title(), invoices: data.cashPurchases, data: data);
      case PurchaseReportType.credit:
        PurchaseReportsPdfService.printInvoiceList(context: context, title: _title(), invoices: data.creditPurchases, data: data);
      case PurchaseReportType.card:
        PurchaseReportsPdfService.printInvoiceList(context: context, title: _title(), invoices: data.cardPurchases, data: data);
      case PurchaseReportType.cheque:
        PurchaseReportsPdfService.printInvoiceList(context: context, title: _title(), invoices: data.chequePurchases, data: data);
      case PurchaseReportType.byProduct:
        PurchaseReportsPdfService.printByProduct(context: context, items: data.byProduct, data: data);
      case PurchaseReportType.byCategory:
        PurchaseReportsPdfService.printByCategory(context: context, items: data.byCategory, data: data);
      case PurchaseReportType.bySupplier:
        PurchaseReportsPdfService.printBySupplier(context: context, items: data.bySupplier, data: data);
      case PurchaseReportType.cancelled:
        PurchaseReportsPdfService.printCancelled(context: context, items: data.cancelledInvoices, data: data);
      case PurchaseReportType.orders:
        PurchaseReportsPdfService.printOrders(context: context, items: data.purchaseOrders, data: data);
      default:
        break;
    }
  }
  ...
  void _onShare(BuildContext context, PurchaseReportsData data) {
    switch (widget.reportType) {
      case PurchaseReportType.all:
        PurchaseReportsPdfService.shareInvoiceList(context: context, title: _title(), invoices: data.allPurchases, data: data);
      case PurchaseReportType.cash:
        PurchaseReportsPdfService.shareInvoiceList(context: context, title: _title(), invoices: data.cashPurchases, data: data);
      case PurchaseReportType.credit:
        PurchaseReportsPdfService.shareInvoiceList(context: context, title: _title(), invoices: data.creditPurchases, data: data);
      case PurchaseReportType.card:
        PurchaseReportsPdfService.shareInvoiceList(context: context, title: _title(), invoices: data.cardPurchases, data: data);
      case PurchaseReportType.cheque:
        PurchaseReportsPdfService.shareInvoiceList(context: context, title: _title(), invoices: data.chequePurchases, data: data);
      case PurchaseReportType.byProduct:
        PurchaseReportsPdfService.shareByProduct(context: context, items: data.byProduct, data: data);
      case PurchaseReportType.byCategory:
        PurchaseReportsPdfService.shareByCategory(context: context, items: data.byCategory, data: data);
      case PurchaseReportType.bySupplier:
        PurchaseReportsPdfService.shareBySupplier(context: context, items: data.bySupplier, data: data);
      case PurchaseReportType.cancelled:
        PurchaseReportsPdfService.shareCancelled(context: context, items: data.cancelledInvoices, data: data);
      case PurchaseReportType.orders:
        PurchaseReportsPdfService.shareOrders(context: context, items: data.purchaseOrders, data: data);
      default:
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isExcel = widget.reportType == PurchaseReportType.excel || widget.reportType == PurchaseReportType.excelProducts;
    ...
    return Scaffold(
          ...
          if (!isExcel)
            BlocBuilder<PurchaseReportsBloc, RealtimeState<PurchaseReportsData>>(
              builder: (context, state) {
                if (state is! RealtimeSuccess<PurchaseReportsData>) return const SizedBox.shrink();
                return Row(
      ...
      body: BlocBuilder<PurchaseReportsBloc, RealtimeState<PurchaseReportsData>>(
        builder: (context, state) {
  ...
  Widget _buildSummary(BuildContext context, PurchaseReportsData data) {
    ...
    switch (widget.reportType) {
      ...
      case PurchaseReportType.cancelled:
        ...
        countLabel = 'reports.invoice_count'.tr();
      case PurchaseReportType.orders:
        totalCents = data.purchaseOrders.fold(0, (sum, i) => sum + i.totalCents);
  ...
  Widget _buildContent(BuildContext context, PurchaseReportsData data) {
    switch (widget.reportType) {
      ...
      case PurchaseReportType.cheque:
        return _InvoiceListView(invoices: data.chequePurchases);
      case PurchaseReportType.byProduct:
      ...
      case PurchaseReportType.excelProducts:
        return _ExcelExportView(
          data: data,
          includeProducts: widget.reportType == PurchaseReportType.excelProducts,
        );
    }
  }

  List<PurchasesByProductItem> _filterProducts(List<PurchasesByProductItem> items) {
  ...
  List<PurchasesByCategoryItem> _filterCategories(List<PurchasesByCategoryItem> items) {
    if (_searchQuery.isEmpty) return items;
    return items.where((c) => c.categoryName.toLowerCase().contains(_searchQuery)).toList();
  }

  List<PurchasesBySupplierItem> _filterSuppliers(List<PurchasesBySupplierItem> items) {
    if (_searchQuery.isEmpty) return items;
    return items.where((s) => s.supplierName.toLowerCase().contains(_searchQuery)).toList();
  }
}
        ...
        child: DataTable(
          ...
          rows: invoices.asMap().entries.map((entry) {
            ...
            return DataRow(cells: [
              ...
              DataCell(Text(p.supplierName, maxLines: 1, overflow: TextOverflow.ellipsis)),
              ...
              DataCell(Text(DateFormat.yMd().format(p.purchaseDate))),
            ]);
          }).toList(),
        ),
      ),
      ...
      child: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          columns: [
            ...
            DataColumn(label: Text('reports.total_purchases'.tr()), numeric: true),
            DataColumn(label: Text('reports.discount'.tr()), numeric: true),
            DataColumn(label: Text('reports.tax'.tr()), numeric: true),
      ...
      child: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          rows: items.asMap().entries.map((entry) {
            ...
            return DataRow(cells: [
              DataCell(Text('$idx')),
              DataCell(Text(s.supplierName, maxLines: 1, overflow: TextOverflow.ellipsis)),
              DataCell(Text(
              ...
              DataCell(Text(
                s.lastPurchaseDate != null ? DateFormat.yMd().format(s.lastPurchaseDate!) : '-',
              )),
            ]);
          }).toList(),
        ),
        ...
        child: DataTable(
          ...
          rows: items.asMap().entries.map((entry) {
            ...
            return DataRow(cells: [
              ...
              DataCell(Text(DateFormat.yMd().format(i.purchaseDate))),
        ...
        child: DataTable(
          ...
          rows: items.asMap().entries.map((entry) {
            ...
            return DataRow(cells: [
              ...
              DataCell(Text(DateFormat.yMd().format(o.purchaseDate))),
              DataCell(Text(o.dueDate != null ? DateFormat.yMd().format(o.dueDate!) : '-')),
            ]);
          }).toList(),
        ),
      ),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    _ExcelStat(
                      label: 'reports.total_purchases'.tr(),
                      value: cs.formatCents(widget.data.summary.totalPurchasesCents),
                    ),
                    _ExcelStat(
