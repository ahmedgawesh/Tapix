...
import '../presentation/bloc/supplier_aging_report_bloc.dart';

class SupplierAgingPdfService {
  static Future<void> printSupplierAgingReport({
    required BuildContext context,
    required SupplierAgingReportData data,
  }) async {
    ...
    await Printing.layoutPdf(
      ...
      name:
          'SupplierAgingReport_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }

  static Future<void> shareSupplierAgingReport({
    required BuildContext context,
    required SupplierAgingReportData data,
  }) async {
    final cs = sl<CurrencyService>();
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename:
          'SupplierAgingReport_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            _buildHeader(
                company, _t('supplier_aging_report', lang), fonts, dir),
            ...
            pw.Row(
              ...
              children: [
                pw.Text(
                  '${_t('total_payables', lang)}: ${cs.formatCents(data.grandTotalCents)}',
                ...
                pw.Text(
                  '${_t('total_suppliers', lang)}: ${data.supplierCount}',
                  style: pw.TextStyle(font: fonts.bold, fontSize: 11),
                ),
              ],
            ),
            ...
            // Supplier aging detail table
            if (data.suppliers.isNotEmpty) ...[
              pw.Text(
                _t('supplier_aging_detail', lang),
                style: pw.TextStyle(font: fonts.bold, fontSize: 12),
              ),
              pw.SizedBox(height: 6),
              pw.TableHelper.fromTextArray(
  ...
  static const _translations = {
    'supplier_aging_report': {
      'en': 'Supplier Aging Report',
      'ar': 'تقرير أعمار ديون الموردين',
      'fr': 'Rapport d\'Ancienneté des Fournisseurs',
    },
    ...
    'supplier_aging_detail': {
      'en': 'Supplier Aging Detail',
      'ar': 'تفاصيل أعمار ديون الموردين',
      'fr': 'Détail d\'Ancienneté par Fournisseur',
    },
