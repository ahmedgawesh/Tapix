...
class CategoryMovementPdfService {
  static Future<void> printReport({
    ...
    await Printing.layoutPdf(
      ...
      name: 'CategoryMovement_${data.selectedCategoryName ?? ""}_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }
  ...
  static Future<void> shareReport({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'CategoryMovement_${data.selectedCategoryName ?? ""}_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
          ...
          return [
            ...
            if (data.productSummaries.isNotEmpty) ...[
              ...
              pw.TableHelper.fromTextArray(
                ...
                data: data.productSummaries.map((ps) => [
                  ps.productName,
                  ps.hasVariants ? _t('with_variants', lang) : _t('without_variants', lang),
                  '+${ps.purchasedQty}',
                  '-${ps.soldQty}',
                  '+${ps.saleReturnedQty}',
                  '-${ps.purchaseReturnedQty}',
                  ps.netQty >= 0 ? '+${ps.netQty}' : '${ps.netQty}',
                ]).toList(),
              ),
              pw.SizedBox(height: 16),
            ],
  ...
  static pw.Widget _buildSummaryTable(
    ...
    return pw.TableHelper.fromTextArray(
      ...
      data: [
        ...
        [_t('net', lang), totals.netQuantity >= 0 ? '+${totals.netQuantity}' : '${totals.netQuantity}', ''],
      ],
    );
  }
  ...
  static (String, String) _movementTypeInfo(CatMovementType type, String lang) {
    switch (type) {
      case CatMovementType.purchase: return (_t('purchases', lang), '+');
      case CatMovementType.sale: return (_t('sales', lang), '-');
      case CatMovementType.saleReturn: return (_t('sale_returns', lang), '+');
      case CatMovementType.purchaseReturn: return (_t('purchase_returns', lang), '-');
    }
  }

  static const _translations = {
    'title': {'en': 'Category Movement Report', 'ar': 'تقرير حركة التصنيف', 'fr': 'Rapport Mouvement Catégorie'},
    'category': {'en': 'Category', 'ar': 'التصنيف', 'fr': 'Catégorie'},
    'products': {'en': 'Active Products', 'ar': 'المنتجات النشطة', 'fr': 'Produits Actifs'},
    'product': {'en': 'Product', 'ar': 'المنتج', 'fr': 'Produit'},
    'period': {'en': 'Period', 'ar': 'الفترة', 'fr': 'Période'},
    'product_breakdown': {'en': 'Product Breakdown', 'ar': 'تفصيل المنتجات', 'fr': 'Détail des Produits'},
  ...
  static pw.Widget _buildHeader(CompanyProfile company, String title, _PdfFonts fonts, pw.TextDirection dir) {
