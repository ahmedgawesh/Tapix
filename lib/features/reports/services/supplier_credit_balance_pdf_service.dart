...
import '../presentation/bloc/supplier_credit_balance_report_bloc.dart';

class SupplierCreditBalancePdfService {
  static Future<void> printSupplierCreditBalanceReport({
    required BuildContext context,
    required SupplierCreditBalanceReportData data,
  }) async {
    ...
    await Printing.layoutPdf(
      ...
      name:
          'SupplierCreditBalanceReport_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }

  static Future<void> shareSupplierCreditBalanceReport({
    required BuildContext context,
    required SupplierCreditBalanceReportData data,
  }) async {
    final cs = sl<CurrencyService>();
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename:
          'SupplierCreditBalanceReport_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Row(
              ...
              children: [
                pw.Text(
                  '${_t('total_credit_balance', lang)}: ${cs.formatCents(data.grandTotalCreditBalanceCents)}',
                  style: pw.TextStyle(font: fonts.bold, fontSize: 11),
                ),
                pw.Text(
            ...
            // Supplier credit balance data table
            if (data.suppliers.isNotEmpty) ...[
              pw.Text(
                _t('supplier_credit_balance_details', lang),
                  ...
                  children: [
                    ...
                    pw.Row(
                      children: [
                        ...
                        pw.SizedBox(width: 16),
                        pw.Text(
                          '${_t('credit_balance', lang)}: ${cs.formatCents(data.grandTotalCreditBalanceCents)}',
  ...
  static const _translations = {
    'supplier_credit_balance_report': {
      'en': 'Supplier Credit Balance Report',
      'ar': 'تقرير أرصدة الموردين الدائنة',
      'fr': 'Rapport des Soldes Créditeurs Fournisseurs',
    },
    ...
    'total_credit_balance': {
      ...
      'ar': 'إجمالي الرصيد الدائن',
      'fr': 'Total Solde Créditeur',
    },
    'total_suppliers': {
      'en': 'Total Suppliers',
      'ar': 'إجمالي الموردين',
    ...
    'supplier_credit_balance_details': {
      'en': 'Supplier Credit Balance Details',
      'ar': 'تفاصيل أرصدة الموردين الدائنة',
      'fr': 'Détails des Soldes Créditeurs Fournisseurs',
    },
