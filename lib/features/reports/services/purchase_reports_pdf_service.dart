...
class PurchaseReportsPdfService {
  ...
  static Future<void> printByProduct({
    ...
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name: 'PurchasesByProduct_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }
  ...
  static Future<void> shareByProduct({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'PurchasesByProduct_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
  ...
  static Future<void> printByCategory({
    ...
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name: 'PurchasesByCategory_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }
  ...
  static Future<void> shareByCategory({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'PurchasesByCategory_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
  ...
  static Future<void> printBySupplier({
    ...
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name: 'PurchasesBySupplier_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }
  ...
  static Future<void> shareBySupplier({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'PurchasesBySupplier_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
  ...
  static Future<void> printCancelled({
    ...
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdf.save(),
      name: 'CancelledPurchases_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }
  ...
  static Future<void> shareCancelled({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'CancelledPurchases_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
  ...
  static Future<void> shareOrders({
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'PurchaseOrders_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
      ...
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          pw.Row(
            ...
            children: [
              pw.Text('${_t('total_purchases', lang)}: ${cs.formatCents(data.summary.totalPurchasesCents)}',
            ...
            pw.TableHelper.fromTextArray(
              ...
              data: invoices.asMap().entries.map((e) {
                ...
                return [
                  ...
                  DateFormat.yMd().format(i.purchaseDate),
                ];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          _buildPeriodLine(data, fonts, lang),
          pw.SizedBox(height: 4),
          pw.Text('${_t('total_purchases', lang)}: ${cs.formatCents(data.summary.totalPurchasesCents)}  |  ${_t('products', lang)}: ${items.length}',
          ...
          if (items.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              headerDecoration: const pw.BoxDecoration(color: PdfColors.grey200),
              headers: ['#', _t('product', lang), _t('category', lang), _t('quantity', lang), _t('total_purchases', lang), _t('discount', lang), _t('tax', lang), _t('invoices', lang)],
              data: items.asMap().entries.map((e) {
                final p = e.value;
                return ['${e.key + 1}', p.productName, p.categoryName ?? '-', '${p.totalQuantity}', cs.formatCents(p.totalPurchasesCents), cs.formatCents(p.totalDiscountCents), cs.formatCents(p.totalTaxCents), '${p.invoiceCount}'];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
          _buildFooter(fonts, lang),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          _buildPeriodLine(data, fonts, lang),
          pw.SizedBox(height: 4),
          pw.Text('${_t('total_purchases', lang)}: ${cs.formatCents(data.summary.totalPurchasesCents)}  |  ${_t('categories', lang)}: ${items.length}',
              style: pw.TextStyle(font: fonts.bold, fontSize: 10)),
          pw.SizedBox(height: 12),
          if (items.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              cellStyle: pw.TextStyle(font: fonts.regular, fontSize: 8),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.grey200),
              headers: ['#', _t('category', lang), _t('products', lang), _t('quantity', lang), _t('total_purchases', lang), _t('discount', lang), _t('tax', lang), _t('invoices', lang)],
              data: items.asMap().entries.map((e) {
                final c = e.value;
                return ['${e.key + 1}', c.categoryName, '${c.productCount}', '${c.totalQuantity}', cs.formatCents(c.totalPurchasesCents), cs.formatCents(c.totalDiscountCents), cs.formatCents(c.totalTaxCents), '${c.invoiceCount}'];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
          _buildFooter(fonts, lang),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          _buildPeriodLine(data, fonts, lang),
          pw.SizedBox(height: 4),
          pw.Text('${_t('total_purchases', lang)}: ${cs.formatCents(data.summary.totalPurchasesCents)}  |  ${_t('suppliers', lang)}: ${items.length}',
              style: pw.TextStyle(font: fonts.bold, fontSize: 10)),
          pw.SizedBox(height: 12),
          if (items.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              cellStyle: pw.TextStyle(font: fonts.regular, fontSize: 8),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.grey200),
              headers: ['#', _t('supplier', lang), _t('total_purchases', lang), _t('discount', lang), _t('tax', lang), _t('invoices', lang), _t('quantity', lang), _t('last_purchase', lang)],
              data: items.asMap().entries.map((e) {
                final s = e.value;
                return ['${e.key + 1}', s.supplierName, cs.formatCents(s.totalPurchasesCents), cs.formatCents(s.totalDiscountCents), cs.formatCents(s.totalTaxCents), '${s.invoiceCount}', '${s.totalQuantity}', s.lastPurchaseDate != null ? DateFormat.yMd().format(s.lastPurchaseDate!) : '-'];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
          _buildFooter(fonts, lang),
        ...
        build: (pw.Context ctx) => [
          ...
          if (items.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              cellStyle: pw.TextStyle(font: fonts.regular, fontSize: 8),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.grey200),
              headers: ['#', _t('purchase_number', lang), _t('supplier', lang), _t('total', lang), _t('date', lang), _t('notes', lang)],
              data: items.asMap().entries.map((e) {
                final i = e.value;
                return ['${e.key + 1}', i.purchaseNumber, i.supplierName, cs.formatCents(i.totalCents), DateFormat.yMd().format(i.purchaseDate), i.notes ?? '-'];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
          _buildFooter(fonts, lang),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context ctx) => [
          ...
          _buildPeriodLine(data, fonts, lang),
          pw.SizedBox(height: 4),
          pw.Text('${_t('total', lang)}: ${cs.formatCents(totalOrders)}  |  ${_t('orders', lang)}: ${items.length}',
              style: pw.TextStyle(font: fonts.bold, fontSize: 10)),
          pw.SizedBox(height: 12),
          if (items.isNotEmpty)
            pw.TableHelper.fromTextArray(
              ...
              cellStyle: pw.TextStyle(font: fonts.regular, fontSize: 8),
              headerDecoration: const pw.BoxDecoration(color: PdfColors.grey200),
              headers: ['#', _t('purchase_number', lang), _t('supplier', lang), _t('total', lang), _t('date', lang), _t('due_date', lang)],
              data: items.asMap().entries.map((e) {
                final o = e.value;
                return ['${e.key + 1}', o.purchaseNumber, o.supplierName, cs.formatCents(o.totalCents), DateFormat.yMd().format(o.purchaseDate), o.dueDate != null ? DateFormat.yMd().format(o.dueDate!) : '-'];
              }).toList(),
            ),
          pw.SizedBox(height: 12),
          _buildFooter(fonts, lang),
  ...
  static pw.Widget _buildPeriodLine(PurchaseReportsData data, _PdfFonts fonts, String lang) {
    return pw.Text(
  ...
  static const _translations = {
    ...
    'total_purchases': {'en': 'Total Purchases', 'ar': 'إجمالي المشتريات', 'fr': 'Total des Achats'},
    ...
    'due_date': {'en': 'Due Date', 'ar': 'تاريخ الاستحقاق', 'fr': 'Date d\'échéance'},
    ...
    'purchases_by_product': {'en': 'Purchases by Product', 'ar': 'المشتريات حسب الصنف', 'fr': 'Achats par Produit'},
    'purchases_by_category': {'en': 'Purchases by Category', 'ar': 'المشتريات حسب التصنيف', 'fr': 'Achats par Catégorie'},
    'purchases_by_supplier': {'en': 'Purchases by Supplier', 'ar': 'المشتريات حسب المورد', 'fr': 'Achats par Fournisseur'},
    'cancelled_purchases': {'en': 'Cancelled Purchases', 'ar': 'مشتريات ملغاة', 'fr': 'Achats Annulés'},
    'purchase_orders': {'en': 'Purchase Orders', 'ar': 'طلبات الشراء', 'fr': 'Bons de Commande'},
  };
