...
import '../presentation/bloc/sales_tax_report_bloc.dart';

class SalesTaxPdfService {
  static Future<void> printSalesTaxReport({
    required BuildContext context,
    required SalesTaxReportData data,
  }) async {
    ...
    await Printing.layoutPdf(
      ...
      name: 'SalesTaxReport_${DateFormat('yyyyMMdd').format(DateTime.now())}',
    );
  }

  static Future<void> shareSalesTaxReport({
    required BuildContext context,
    required SalesTaxReportData data,
  }) async {
    final cs = sl<CurrencyService>();
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'SalesTaxReport_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            _buildHeader(company, _t('sales_tax_report', lang), fonts, dir),
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  ...
                  _summaryCol(_t('tax_collected', lang), cs.formatCents(data.totalTaxCollectedCents), fonts),
            ...
            if (data.invoices.isNotEmpty) ...[
              pw.Text(_t('sales_invoices', lang),
              ...
              pw.TableHelper.fromTextArray(
                ...
                data: data.invoices.asMap().entries.map((entry) {
                  ...
                  return [
                    ...
                    inv.invoiceNumber,
                    inv.customerName ?? '-',
                    DateFormat.yMd().format(inv.saleDate),
                ...
                child: pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(
                      ...
                      '${_t('tax', lang)}: ${cs.formatCents(data.totalTaxCollectedCents)}  |  '
                      '${_t('total', lang)}: ${cs.formatCents(data.totalSalesCents)}',
                      style: pw.TextStyle(font: fonts.bold, fontSize: 10),
                    ),
                  ],
                ),
  ...
  static const _translations = {
    'sales_tax_report': {
      'en': 'Sales Tax Report',
      'ar': 'تقرير ضرائب المبيعات',
      'fr': 'Rapport de Taxe sur les Ventes',
    },
