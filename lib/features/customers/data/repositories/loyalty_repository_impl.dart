...
class LoyaltyRepositoryImpl implements LoyaltyRepository {
  ...
  Stream<List<LoyaltyTier>> watchAllTiers() {
    return (_database.select(
      _database.loyaltyTiers,
    )..orderBy([(t) => OrderingTerm(expression: t.minPoints)])).watch();
  }
  ...
  Future<List<LoyaltyTier>> getAllTiers() {
    return (_database.select(
      _database.loyaltyTiers,
    )..orderBy([(t) => OrderingTerm(expression: t.minPoints)])).get();
  }
  ...
  Stream<CustomerLoyaltySummary?> watchCustomerLoyaltySummary(int customerId) {
    ...
    return _database.customerDao.watchCustomer(customerId).asyncMap((
      customer,
    ) async {
      ...
      final transactions = await (_database.select(
        _database.loyaltyPointTransactions,
  ...
  Stream<List<LoyaltyPointTransaction>> _watchPointTransactionsInternal(
    int customerId,
  ) {
  ...
  @override
  Future<CustomerLoyaltySummary?> getCustomerLoyaltySummary(
    int customerId,
  ) async {
  ...
  Future<void> addPoints({
    ...
    await _database
        .into(_database.loyaltyPointTransactions)
        .insert(
  ...
  Future<List<LoyaltyReward>> getAvailableRewards(int customerId) async {
    ...
    final rewards = await (_database.select(
      _database.loyaltyRewards,
  ...
  Future<void> redeemReward({
    ...
    final reward = await (_database.select(
      _database.loyaltyRewards,
    ...
    // Record redemption
    final redemptionId = await _database
        .into(_database.customerRewardRedemptions)
        .insert(
    ...
    await (_database.update(
      _database.loyaltyRewards,
    )..where((r) => r.id.equals(rewardId))).write(
  ...
  Stream<List<CustomerRewardRedemption>> watchCustomerRedemptions(
    ...
    return (_database.select(_database.customerRewardRedemptions)
          ...
          ..orderBy([
            (r) =>
                OrderingTerm(expression: r.redeemedAt, mode: OrderingMode.desc),
          ]))
        .watch();
  }
  ...
  Future<void> _reassignAllCustomerTiers() async {
    ...
    final tiers =
        await (_database.select(_database.loyaltyTiers)
    ...
    final customers = await (_database.select(
      _database.customers,
    )..where((c) => c.loyaltyEnabled.equals(true))).get();
  ...
  @override
  Future<List<LoyaltyPointTransaction>> getPointsTransactions(
    int customerId,
  ) async {
    return (_database.select(_database.loyaltyPointTransactions)
          ..where((t) => t.customerId.equals(customerId))
