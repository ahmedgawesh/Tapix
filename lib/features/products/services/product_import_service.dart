...
class ProductImportService implements ImportProducts {
  ...
  Future<ImportResult> call({
    ...
    for (var rowIndex = 0; rowIndex < fileData.rows.length; rowIndex++) {
      ...
      try {
        parsedRows.add(
          _parseRow(row, rowIndex, columnMapping, fileData.headers),
        );
      } catch (e) {
        errors.add(
          ImportError(
            ...
            if (!hasVariants) {
              ...
              for (final r in rows) {
                ...
                if (r.color.isNotEmpty) {
                  ...
                  if (colorId == null) {
                    final createdId = await _variantRepository.createColor(
                      r.color,
                      null,
                    );
                ...
                if (r.size.isNotEmpty) {
                  ...
                  if (sizeId == null) {
                    final createdId = await _variantRepository.createSize(
                      r.size,
                      0,
                      null,
                    );
  ...
  _ParsedRow _parseRow(
    ...
    final isTaxable =
        isTaxableStr.toLowerCase() == 'true' || isTaxableStr == '1';
    ...
    final hasVariants =
        hasVariantsStr.toLowerCase() == 'true' || hasVariantsStr == '1';
  ...
  Decimal _parseMoneyLikeToCents(String value, {required String? header}) {
