...
class Commissions extends Table {
  ...
  IntColumn get commissionRateBps =>
      integer().map(const BasisPointsConverter())();
  IntColumn get commissionAmountCents =>
...
class PayrollDeductions extends Table {
  IntColumn get id => integer().autoIncrement()();
  IntColumn get payrollId =>
      integer().references(Payrolls, #id, onDelete: KeyAction.cascade)();
...
class EmployeeDocuments extends Table {
  ...
  IntColumn get uploadedBy => integer().nullable().references(
    Users,
    #id,
    onDelete: KeyAction.setNull,
  )();
...
class OvertimeRules extends Table {
  ...
  RealColumn get dailyHoursThreshold =>
      real().withDefault(const Constant(8.0))();

  /// Weekly hours threshold
  RealColumn get weeklyHoursThreshold =>
...
class PerformanceMetrics extends Table {
  ...
  IntColumn get recordedBy => integer().nullable().references(
    Users,
    #id,
    onDelete: KeyAction.setNull,
  )();
